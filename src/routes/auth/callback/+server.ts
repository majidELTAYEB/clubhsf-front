...
// 		console.log({ code, state, storedState, verifier });
...
// 	console.log('Token exchange failed:', res.status, errBody);
// 	throw error(401, "Échec de l'échange du code");
// }
...
export const GET: RequestHandler = async ({ url, cookies, fetch, request }) => {
	...
	// sinon fallback sur url.origin (accès direct sans proxy)
	// Doit rester cohérent avec le calcul fait dans startAuth0Flow
	...
	const USER_PROFILE_MAX_AGE = 60 * 60 * 24 * 30;
	cookies.set('user_profile', JSON.stringify(me), { ...secureCookie, maxAge: USER_PROFILE_MAX_AGE });
