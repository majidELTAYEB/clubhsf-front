...
// 	// ⚠️ hypothèse sur le chemin, comme le reste de ton API (/api/v1/...) — à corriger si besoin.
...
// 	}
// 	// Profil mis en cache côté cookie (role + is_premium) pour éviter un fetch /me à chaque requête
...
export const GET: RequestHandler = async ({ url, cookies, fetch }) => {
	...
	// Doit être identique, caractère pour caractère, au redirect_uri envoyé
	// lors de l'appel /authorize — recalculé ici de la même façon.
	...
	// Server-to-server (SvelteKit → backend), pas navigateur → téléphone,
	// donc localhost:8081 reste valide même si le navigateur est sur le réseau.
	const meRes = await fetch('http://localhost:8080/api/v1/me', {
		headers: { Authorization: `Bearer ${tokens.access_token}` }
	});
