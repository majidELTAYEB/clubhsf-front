...
{#snippet credentialField(label: string, value: string, field: string)}
	<div class="grid gap-1.5">
		...
		<div class="flex items-center gap-2">
			<code class="flex-1 truncate border border-border bg-muted px-3 py-2 text-sm">{value}</code>
			<Button variant="outline" size="icon" class="shrink-0" onclick={() => copy(value, field)}>
				{#if copiedField === field}
					<CheckIcon class="size-4 text-green-600" />
				{:else}
...
<div class="mx-auto w-full w-full space-y-8 px-4 py-10 sm:px-6 lg:px-8">
	{#if !live}
		<LiveError message="Ce live n'existe pas ou a été supprimé." onRetry={() => location.reload()} />
	{:else}
		<div class="flex flex-col gap-6 lg:flex-row lg:items-stretch">
			<div class="min-w-0 flex-1">
				{#if isLive}
					<div class="h-full overflow-hidden border border-border">
						<LivePlayer playbackId={live.playback_id} title={live.title} />
					</div>
				{:else if isScheduled}
					<LiveUpcoming title={live.title} coverSrc={live.cover_url} scheduleAt={live.schedule_at} />
		...
		<div class="w-full space-y-3 border-t border-border pt-6">
			<div class="flex flex-wrap items-center justify-between gap-2">
				<h1 class="text-2xl font-bold tracking-tight text-foreground">{live.title}</h1>
			</div>
			...
			<div class="flex flex-wrap items-center gap-2">
				...
				{#if live.schedule_at}
					<span class="text-sm text-muted-foreground">{formattedDate(live.schedule_at)}</span>
				{/if}
			</div>

			{#if live.description}
				<div class="w-full border border-border bg-muted/50 p-3">
					<p class="text-sm whitespace-pre-line text-foreground/90">{live.description}</p>
				</div>
			{/if}
		</div>
