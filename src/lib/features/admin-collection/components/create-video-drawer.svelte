...
// import { getCollections, getUploadCredentials } from "$lib/features/new-video/api";
    ...
    const totalSteps = $derived(lockedCollection ? 2 : 3);
    // Affichage uniquement : en interne `step` garde les valeurs 1 et 3
    // pour ne pas dupliquer toute la logique des étapes.
    const displayStep = $derived(lockedCollection && step === 3 ? 2 : step);
    ...
    <div class="grid gap-4">
        {#if lockedCollection}
            <Badge variant="secondary" class="w-fit gap-1.5 font-normal">
                ...
                Ajoutée à {collectionTitle ?? "cette collection"}
            </Badge>
        {/if}
