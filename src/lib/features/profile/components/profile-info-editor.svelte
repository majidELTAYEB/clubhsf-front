...
<script lang="ts">
	...
	function extractErrorMessage(err: unknown, fallback: string): string {
		// Le backend renvoie généralement { code, message } ; on essaie de
		// remonter ce message précis avant de retomber sur un message générique.
	...
	async function saveInfo() {
		...
		// On garde en local la valeur nettoyée pour éviter d'envoyer/afficher
		// des espaces superflus, sans pour autant reformater ce que l'utilisateur voit ailleurs.
		...
		try {
			...
			errorMsg = extractErrorMessage(err, 'Échec de la sauvegarde');
		} finally {
			saving = false;
		}
	}
	...
	async function addSocialLink() {
		...
		try {
			...
			errorMsg = extractErrorMessage(err, "Échec de l'ajout");
		} finally {
			addingSocial = false;
		}
	}
	...
	async function removeSocialLink(platform: string) {
		...
		try {
			...
			errorMsg = extractErrorMessage(err, 'Échec de la suppression');
		} finally {
			removingPlatform = null;
		}
	}
...
<div class="field-group">
	...
	<label class="field">
		<span>Nom d'utilisateur</span>
		<input type="text" bind:value={username} aria-invalid={errorMsg !== null} />
	</label>
	<label class="field">
		<span>Bio</span>
		<textarea bind:value={bio} rows="3"></textarea>
