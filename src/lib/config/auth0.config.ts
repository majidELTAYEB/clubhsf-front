...
// Le chemin ne change jamais entre environnements (dev, réseau local, prod) —
// seule l'origine (protocole + host + port) change, et elle est recalculée
// dynamiquement depuis `url.origin` dans login+server.ts / callback+server.ts.
// Plus besoin d'une variable d'env pour ça.
...
export const AUTH0_M2M_CLIENT_ID = required('AUTH0_M2M_CLIENT_ID', privateEnv.AUTH0_M2M_CLIENT_ID);
export const AUTH0_M2M_CLIENT_SECRET = required('AUTH0_M2M_CLIENT_SECRET', privateEnv.AUTH0_M2M_CLIENT_SECRET);
